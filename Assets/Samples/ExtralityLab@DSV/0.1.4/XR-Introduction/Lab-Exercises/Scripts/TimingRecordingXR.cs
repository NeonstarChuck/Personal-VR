...
public class TimingRecordingXR : MonoBehaviour
{
    ...
    bool m_IsTiming;
    BaseInteractivePuzzlePieceXR[] m_PuzzlePieces;

    void Awake ()
    {
        m_PuzzlePieces = FindObjectsByType<BaseInteractivePuzzlePieceXR>(FindObjectsSortMode.InstanceID);
    ...
    void Start()
    {
        // Enable Control after Fading in
        Invoke(nameof(EnableControl),ScreenFader.Instance.fadeDuration);
    }
